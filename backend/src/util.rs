...
/// 对 append-only 文件做增量行读取：每次从上次偏移处读取新增字节，
/// 拼接进 remainder 缓冲；只有以 `\n` 结尾的完整行才会输出，末尾的
/// 半行留在缓冲中等待下次补齐（写入方可能正在写半行 JSON）。
/// 文件被截断/轮转（长度小于偏移）时自动重置。
#[derive(Default)]
...
impl LineReader {
    ...
    /// 读取自上次调用以来新增的完整行。
    pub fn read_since(&mut self, path: &std::path::Path) -> std::io::Result<Vec<String>> {
...
mod tests {
    ...
    fn temp_file(name: &str) -> std::path::PathBuf {
        ...
        p.push(format!("atv-line-reader-{}-{name}", std::process::id()));
        std::fs::write(&p, b"").unwrap();
        p
    }

    #[test]
    fn line_reader_incremental_and_partial() {
        use std::io::Write;
        let path = temp_file("inc");
        let mut r = LineReader::new();

        std::fs::write(&path, "line1\nline2\n").unwrap();
        assert_eq!(r.read_since(&path).unwrap(), vec!["line1", "line2"]);
        ...
        std::fs::OpenOptions::new()
            ...
            .write_all(b"line3\n")
            .unwrap();
        assert_eq!(r.read_since(&path).unwrap(), vec!["line3"]);
        ...
        std::fs::OpenOptions::new()
            ...
            .write_all(b"half")
            .unwrap();
        assert!(r.read_since(&path).unwrap().is_empty());
        ...
        std::fs::OpenOptions::new()
            ...
            .write_all(b"-done\n")
            .unwrap();
        assert_eq!(r.read_since(&path).unwrap(), vec!["half-done"]);
    ...
    #[test]
    fn line_reader_truncation_resets() {
        let path = temp_file("trunc");
        let mut r = LineReader::new();

        std::fs::write(&path, "aaa\nbbb\n").unwrap();
        assert_eq!(r.read_since(&path).unwrap(), vec!["aaa", "bbb"]);

        // 文件被截断重写（长度小于已读偏移）→ 从头重读
        std::fs::write(&path, "cc\n").unwrap();
        assert_eq!(r.read_since(&path).unwrap(), vec!["cc"]);
        std::fs::remove_file(&path).ok();
    }
